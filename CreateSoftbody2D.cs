...
public class CreateSoftbody2D : MonoBehaviour
{
    ...
    static void MakeSoftbody()
    {
        ...
        {
            Debug.LogError("Create Softbody 2D :: Current selection does not have a \"Sprite Skin\" component.");
            return;
        }
        if(_spriteSkinScr.rootBone == null || _spriteSkinScr.boneTransforms.Length == 0)
        {
            Debug.LogError("Create Softbody 2D :: Current selection does not have any bone objects.");
            return;
        }
        
        //warn for uneven # of bones
        if(_spriteSkinScr.boneTransforms.Length % 2 != 0) Debug.LogWarning("Create Softbody 2D :: Selected object has an uneven number of bones. Unexpected physics behaviors may occur.");
        
        //add rigidbodies and colliders
        Transform[] _bones = _spriteSkinScr.boneTransforms;
        foreach(Transform child in _bones)
        {
            if(!child.GetComponent<Rigidbody2D>()) child.AddComponent<Rigidbody2D>();
            if(!child.GetComponent<CircleCollider2D>())
            {
                CircleCollider2D _col = child.AddComponent<CircleCollider2D>();

                _col.radius = 0.04f;
                _col.offset = new Vector2(0.01f, 0);
            }
        }
        ...
        for(int j = 0; j < _bones.Length; j++)
        {
            ...
            _leftSpring.autoConfigureDistance = _rightSpring.autoConfigureDistance = _oppositeSpring.autoConfigureDistance = true;
            _leftSpring.autoConfigureConnectedAnchor = _rightSpring.autoConfigureConnectedAnchor = _oppositeSpring.autoConfigureConnectedAnchor = false;
        ...
        Debug.Log("Create Softbody 2D :: Selected object skinned successfully!");
    }
}
